...
public class DBPartOrder implements IFDBPartOrder {
	...
	public int insertPartOrder(PartOrder pOrder) throws Exception {
		...
		pOrder.getProduct().setStock(
				pOrder.getProduct().getStock() - pOrder.getAmount());
	...
	public int updatePartOrder(PartOrder pOrder) {
		...
		try {
			DBConnection.startTransaction();
			String query = "UPDATE PARTORDER SET" + " productID = ?,"
					+ " amount = ?" + " unitPrice = ?" + " WHERE orderID = ?";
			PreparedStatement stmt = con.prepareStatement(query);
			stmt.setQueryTimeout(5);
	...
	public int removePartOrder(PartOrder pOrder) {
		int rc = -1;
		String query = "DELETE FROM PARTORDER WHERE ORDERID = "
				+ pOrder.getParent().getOrderID();
